...
class ControllerManager:
        ...
        while True:
            if time.time() - t_prev > self.controller_dt:  # apply control at the specified controller_dt
                ...
                if self.TAKEOFF:
                    ...
                    if node_OdomVIO.initialization_status and self.TAKEOFF_STARTED and node_OdomVIO.first_pf_pos_msg:
                ...
                if (not (node_OdomVIO.state_dict['mode'] == "GUIDED" or node_OdomVIO.state_dict['mode'] == "GUIDED_NOGPS") or self.DONE) and (self.generated_traj is not None):
                    ...
                    np.save('logs/PF_pos_list_{}.npy'.format(self.date_var),   np.array(self.PF_pos_list))
    ...
    def _track_trajectory(self, node_OdomVIO, node_PixhawkCMD):
        ...
        self.PF_pos_list.append(PF_pos[0:2].copy())
    ...
    def _print_status(self, PF_pos, VIO_vel, ref_pos, ref_vel, a_n, a_e, yaw_target, pitch_target, roll_target):
        if self.print:
            if self.last_print_time + 1 < time.time():
                print(f"diff_pos: {np.round(ref_pos - PF_pos, 4)} | diff_vel: {np.round(ref_vel - VIO_vel, 4)}\n"
                      f"VIO pos : {np.round(PF_pos, 4)}    VIO vel : {np.round(VIO_vel, 4)}\n"
